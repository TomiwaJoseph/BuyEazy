...
def add_to_wishlist(request):
    ...
    if check_wishlist_exist:
        check_product_exist = product_id in [
...
def add_to_cart(request):
    ...
    if request.user.is_authenticated:
        total_items = OrderItem.objects.filter(
            user=request.user, ordered=False).count()
    else:
...
def show_cart_preview(request):
    try:
        if request.user.is_authenticated:
            all_user_order_item_ids = [i.product.id for i in OrderItem.objects.filter(
                user=request.user, ordered=False)]
            cart_contents = [Product.objects.get(
                id=i) for i in all_user_order_item_ids]
        else:
            session_cart = request.session['cart_data']
            all_cart_product_ids = [i for i in session_cart.keys()]
            cart_contents = [Product.objects.get(
                id=i) for i in all_cart_product_ids]
    except KeyError:
        cart_contents = []

    t = render_to_string('shop/ajax_pages/cart_content.html',
...
def load_more_products(request):
    ...
    t = render_to_string('shop/ajax_pages/more_products.html',
...
def delete_cart_item(request):
    ...
    if request.user.is_authenticated:
        orderItem_to_delete = OrderItem.objects.get(
            user=request.user, product__id=request.GET['id'], ordered=False)
        ...
        if order_qs.exists():
            context.update(
                {"totalItems": order_qs[0].get_unordered_cart_items_count})
...
def update_cart_item(request):
    ...
    if request.user.is_authenticated:
        ...
        orderItem_qs = OrderItem.objects.get(
            user=request.user, product__id=product_id, ordered=False)
...
def filter_category(request):
    ...
    filtered_products = Product.objects.filter(
        category__title=category_to_filter)
    t = render_to_string('shop/ajax_pages/filter_products.html',
...
def filter_price_and_product(request):
    ...
    t = render_to_string('shop/ajax_pages/filter_price_product.html',
                         {'filtered_products': filtered_products})
...
class CheckoutView(LoginRequiredMixin, View):
    def get(self, *args, **kwargs):
        ...
        if default_billing_address_qs.exists():
            context.update(
                {'default_billing_address': default_billing_address_qs[0]})
    ...
    def post(self, *args, **kwargs):
        ...
        if form.is_valid():
            ...
            know_if_to_save_shipping_address_as_default = form.cleaned_data.get(
                'save_shipping_address')
            know_if_to_save_billing_address_as_default = form.cleaned_data.get(
                'save_billing_address')
            know_if_to_use_default_shipping = form.cleaned_data.get(
                'use_default_shipping')
            know_if_to_use_default_billing = form.cleaned_data.get(
                'use_default_billing')
            ...
            if know_if_to_use_default_shipping and know_if_to_use_default_billing:
                ...
                # check if any address exist for user already and delete
                check_if_address_exist = Address.objects.filter(
                    user=self.request.user)
                if check_if_address_exist.exists():
                    Address.objects.get(
                ...
                if know_if_to_save_billing_address_as_default:
                    Address.objects.filter(
                        user=self.request.user, address_type='B', default=True).update(default=False)
                    ...
                    new_billing_address.save()
                                                                    ...
                                                                    billing_country, billing_zip_code]):
                # delete the already existing Billing Address
                Address.objects.get(
                    user=self.request.user, address_type="B").delete()
                user_shipping_address = Address.objects.get(
                    user=self.request.user, default=True, address_type='S')
                ...
                # delete the already existing Shipping Address
                Address.objects.get(
                    user=self.request.user, address_type="S").delete()
                user_billing_address = Address.objects.get(
                    user=self.request.user, default=True, address_type='B')
                ...
                if know_if_to_save_shipping_address_as_default:
                    Address.objects.filter(
                        user=self.request.user, address_type='S', default=True).update(default=False)
                    new_shipping_address.default = True
                    new_shipping_address.save()
...
class SingleProduct(DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context['similar_products'] = Product.objects.filter(
            category=qs.category).exclude(id=qs.id)[:4]
        context['related_products'] = Product.objects.all()
        context['all_other_images'] = ProductImages.objects.filter(
            product_id=context['object'])
        context['all_reviews'] = Reviews.objects.filter(
            product_id=context['object'])
        return context
    ...
    def post(self, *args, **kwargs):
        ...
        if form.is_valid():
            ...
            try:
                ...
                messages.info(
                    self.request, "You don't have an order with the reference code provided.")
                return redirect("request_refund")
